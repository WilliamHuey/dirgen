module.exports = function(__, lab, cliEntryFile, exec, fs) {
  lab.experiment('and for display messages', function() {
    ...
    lab.test('with error messages and no silent flag will show console warnings', function(done) {
      fs.mkdirAsync(__dirname + '/case-outputs/errors-no-silent-flag')
      .then(function() {

        exec(cliEntryFile + ' g ' +
        'tests/fixtures/errors-no-silent-flag.txt ' +
        ' tests/case-outputs/errors-no-silent-flag', function(error, stdout, stderr) {
    ...
    lab.test('with error messages and with silent flag will not show console warnings', function(done) {
      fs.mkdirAsync(__dirname + '/case-outputs/errors-with-silent-flag')
      .then(function() {

        exec(cliEntryFile + ' g ' +
        'tests/fixtures/errors-with-silent-flag.txt ' +
        ' tests/case-outputs/errors-with-silent-flag -s', function(error, stdout, stderr) {
